...
\begin{example}{}{}
  ...
  \end{center}
  \vspace{0.5cm}
  Dans la base $(\Vecteur{i},\Vecteur{j})$, $\Vecteur{u}$ a ainsi pour coordonnées $\coord{3}{2}$.
\end{example}


\begin{propriete}{Coordonnées d'un vecteur}{}
  Dans un repère, si deux points $A$ et $B$ ont pour coordonnées respectives $(x_A;y_A)$ et $(x_B;y_B)$, le vecteur $\Vecteur{AB}$ a pour coordonnées $\coord{x_B-x_A}{y_B-y_A}$.
\end{propriete}
\begin{demonstration}{}{}
  Soit $M(x;y)$ le point tel que $\Vecteur{OM}=\Vecteur{AB}$. $ABMO$ est un parallélogramme et $[OB]$ et $[AM]$ ont même milieu.\\Les coordonnées du milieu de $[OB]$ sont $\left(\frac{x_B}{2};\frac{y_B}{2}\right)$.\\Les coordonnées du milieu de $[AM]$ sont $\left(\frac{x_A+x}{2};\frac{y_A+y}{2}\right)$. \\
  On en déduit que $\frac{x_B}{2}=\frac{x_A+x}{2}$ et $\frac{y_B}{2}=\frac{y_A+y}{2}$, d'où $x=x_B-x_A$ et $y=y_B-y_A$.
\end{demonstration}
\vspace{0.5cm}
\begin{example}{}{}
  \begin{center}
  ...
  \end{center}
  \vspace{0.5cm}
  Sur le graphique ci-dessus, on a : $A(1;3)$ et $B(4;1)$. Les coordonnées du vecteur $\Vecteur{AB}$ sont donc $\coord{4-1}{1-3}$ soit $\Vecteur{AB}=\coord{3}{-2}$
\end{example}
\vspace{0.5cm}
...
\begin{demonstration}{}{}
  \begin{tabular}{p{12cm} p{4cm}}
    \begin{minipage}{12cm}
      ...
      Les coordonnées de $\Vecteur{MN}$ sont $(x_N-x;y_N-y)$. Or, $\Vecteur{MN}=\Vecteur{v}$, c'est-à-dire $x_N-x=x'$ et $y_N-y=y'$. On en déduit que $N$ a pour coordonnées $(x+x';y+y')$, d'où $\Vecteur{u}+\Vecteur{v}\coord{x+x'}{y+y'}$.
    \end{minipage}
    &\begin{minipage}{4cm}
